...
import { fetchRoadmap, type RoadmapStep as ApiRoadmapStep } from "@/lib/api"
...
interface RoadmapViewProps {
  ...
  /** Pass these to fetch a personalised AI-generated roadmap */
  userId?: string
  scenarioId?: string
}
...
export function RoadmapView({
  ...
  const [aiRoadmapSteps, setAiRoadmapSteps] = useState<ApiRoadmapStep[] | null>(null)
  ...
  // Load hardcoded fallback steps
  useEffect(() => {
    const steps = generateRoadmap(scenario, userResponses)
    setRoadmapSteps(steps)
  }, [scenario, userResponses])

  // Fetch AI-personalised roadmap when userId + scenarioId are provided
  useEffect(() => {
    if (!userId || !scenarioId) return
    setAiLoading(true)
    fetchRoadmap(userId, scenarioId)
      .then((steps) => {
        if (steps.length > 0) setAiRoadmapSteps(steps)
      })
      .catch(() => {
        // Silently fall back to static roadmap on error
      })
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      {/* AI-Personalised Roadmap panel */}
      {(aiLoading || aiRoadmapSteps) && (
        <Card className="border-primary/30 bg-primary/5">
          <CardHeader>
            ...
            {!aiLoading && (
              <p className="text-sm text-muted-foreground">Generated by AI based on your conversation</p>
            )}
          </CardHeader>
          {!aiLoading && aiRoadmapSteps && (
            <CardContent className="space-y-3">
              {aiRoadmapSteps.map((step, i) => (
                <div key={i} className="flex items-start gap-3 p-3 rounded-lg bg-background border">
                  <div
                    className={`mt-0.5 h-5 w-5 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-bold text-white ${
                      step.priority === "high"
                        ? "bg-red-500"
